 ...
 * \ingroup my11s
 * 
 * \brief Helper to create IEEE 802.11s wmn networks
 */
class WmnHelper
{
public:
  /**
   * Construct a WmnHelper used to make life easier when creating 802.11s networks.
   */
  WmnHelper ();
   ...
   *  channels will be assigned to different wmn point interfaces.
   */
  enum ChannelPolicy
  {
    SPREAD_CHANNELS,
  ...
  /**
   * \brief Set a number of interfaces in a wmn network
   * \param nInterfaces is the number of interfaces
   */
  void SetNumberOfInterfaces (uint32_t nInterfaces);

  /** 
   * \brief Install 802.11s wmn device & protocols on given node list
   * 
   * \param phyHelper           Wifi PHY helper
   * \param c               List of nodes to install
   * 
   * \return list of created wmn point devices, see WmnPointDevice
   */
  NetDeviceContainer
  Install (const WifiPhyHelper &phyHelper, NodeContainer c) const;
  /**
   * \param type the type of ns3::WmnStack.
   * \param n0 the name of the attribute to set
   * \param v0 the value of the attribute to set
   * \param n1 the name of the attribute to set
   * \param v1 the value of the attribute to set
   ...
   * \param c NetDeviceContainer of the set of devices for which the wmn devices
