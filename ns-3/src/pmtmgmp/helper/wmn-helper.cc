...
void
WmnHelper::SetSpreadInterfaceChannels (enum ChannelPolicy policy)
{
  m_spreadChannelPolicy = policy;
}
void
WmnHelper::SetStackInstaller (std::string type,
                               ...
                               std::string n7, const AttributeValue &v7)
{
  ...
  m_stack = m_stackFactory.Create<WmnStack> ();
  if (m_stack == 0)
    {
...
void
WmnHelper::SetNumberOfInterfaces (uint32_t nInterfaces)
{
  m_nInterfaces = nInterfaces;
}
NetDeviceContainer
WmnHelper::Install (const WifiPhyHelper &phyHelper, NodeContainer c) const
{
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      // Create a wmn point device
      Ptr<WmnPointDevice> mp = CreateObject<WmnPointDevice> ();
      node->AddDevice (mp);
...
void
WmnHelper::SetMacType (std::string n0, const AttributeValue &v0,
                        std::string n1, const AttributeValue &v1,
                        std::string n2, const AttributeValue &v2,
                        std::string n3, const AttributeValue &v3,
                        std::string n4, const AttributeValue &v4,
                        ...
                        std::string n7, const AttributeValue &v7)
{
  m_mac.SetTypeId ("ns3::WmnWifiInterfaceMac");
  m_mac.Set (n0, v0);
  m_mac.Set (n1, v1);
  m_mac.Set (n2, v2);
  m_mac.Set (n3, v3);
...
void
WmnHelper::SetRemoteStationManager (std::string type,
                                     std::string n0, const AttributeValue &v0,
                                     std::string n1, const AttributeValue &v1,
                                     std::string n2, const AttributeValue &v2,
                                     std::string n3, const AttributeValue &v3,
...
void 
WmnHelper::SetStandard (enum WifiPhyStandard standard)
{
  m_standard = standard;
}

Ptr<WifiNetDevice>
WmnHelper::CreateInterface (const WifiPhyHelper &phyHelper, Ptr<Node> node, uint16_t channelId) const
{
  Ptr<WifiNetDevice> device = CreateObject<WifiNetDevice> ();

  Ptr<WmnWifiInterfaceMac> mac = m_mac.Create<WmnWifiInterfaceMac> ();
  NS_ASSERT (mac != 0);
  mac->SetSsid (Ssid ());
...
void
WmnHelper::Report (const ns3::Ptr<ns3::NetDevice>& device, std::ostream& os)
{
  NS_ASSERT (m_stack != 0);
  Ptr<WmnPointDevice> mp = device->GetObject<WmnPointDevice> ();
  NS_ASSERT (mp != 0);
  std::vector<Ptr<NetDevice> > ifaces = mp->GetInterfaces ();
  os << "<WmnPointDevice time=\"" << Simulator::Now ().GetSeconds () << "\" address=\""
     << Mac48Address::ConvertFrom (mp->GetAddress ()) << "\">\n";
  m_stack->Report (mp, os);
  os << "</WmnPointDevice>\n";
}
void
WmnHelper::ResetStats (const ns3::Ptr<ns3::NetDevice>& device)
{
  NS_ASSERT (m_stack != 0);
  Ptr<WmnPointDevice> mp = device->GetObject<WmnPointDevice> ();
  NS_ASSERT (mp != 0);
...
int64_t
WmnHelper::AssignStreams (NetDeviceContainer c, int64_t stream)
{
  ...
  for (NetDeviceContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      netDevice = (*i);
      Ptr<WmnPointDevice> mpd = DynamicCast<WmnPointDevice> (netDevice);
      Ptr<WifiNetDevice> wifi;
      Ptr<WmnWifiInterfaceMac> mac;
      if (mpd)
        {
          ...
          for (std::vector<Ptr<NetDevice> >::iterator i = ifaces.begin (); i != ifaces.end (); i++)
            {
              ...
              // Handle any random numbers in the wmn mac and plugins
              mac = DynamicCast<WmnWifiInterfaceMac> (wifi->GetMac ());
              if (mac)
                {
