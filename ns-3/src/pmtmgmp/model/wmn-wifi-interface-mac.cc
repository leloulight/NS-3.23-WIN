...
NS_LOG_COMPONENT_DEFINE ("WmnWifiInterfaceMac");

NS_OBJECT_ENSURE_REGISTERED (WmnWifiInterfaceMac);

TypeId
WmnWifiInterfaceMac::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::WmnWifiInterfaceMac")
    .SetParent<RegularWifiMac> ()
    .SetGroupName ("Wmn")
    .AddConstructor<WmnWifiInterfaceMac> ()
    .AddAttribute ( "BeaconInterval",
                    "Beacon Interval",
                    TimeValue (Seconds (0.5)),

                    MakeTimeAccessor (
                      &WmnWifiInterfaceMac::m_beaconInterval),
                    MakeTimeChecker ()
                    )
    .AddAttribute ( "RandomStart",
                    ...
                    TimeValue (Seconds (0.5)),
                    MakeTimeAccessor (
                      &WmnWifiInterfaceMac::m_randomStart),
                    MakeTimeChecker ()
                    )
    .AddAttribute ( "BeaconGeneration",
                    ...
                    BooleanValue (true),
                    MakeBooleanAccessor (
                      &WmnWifiInterfaceMac::SetBeaconGeneration, &WmnWifiInterfaceMac::GetBeaconGeneration),
                    MakeBooleanChecker ()
                    )
  ;
  return tid;
}
WmnWifiInterfaceMac::WmnWifiInterfaceMac () :
...
//-----------------------------------------------------------------------------
void
WmnWifiInterfaceMac::Enqueue (Ptr<const Packet> packet, Mac48Address to, Mac48Address from)
{
  NS_LOG_FUNCTION (this << packet << to << from);
  ForwardDown (packet, from, to);
}
void
WmnWifiInterfaceMac::Enqueue (Ptr<const Packet> packet, Mac48Address to)
{
  NS_LOG_FUNCTION (this << packet << to);
  ForwardDown (packet, m_low->GetAddress (), to);
}
bool
WmnWifiInterfaceMac::SupportsSendFrom () const
{
  return true;
}
void
WmnWifiInterfaceMac::SetLinkUpCallback (Callback<void> linkUp)
{
  NS_LOG_FUNCTION (this);
  RegularWifiMac::SetLinkUpCallback (linkUp);

  // The approach taken here is that, from the point of view of a wmn
  // node, the link is always up, so we immediately invoke the
  // callback if one is set
  linkUp ();
}
void
WmnWifiInterfaceMac::DoDispose ()
{
  NS_LOG_FUNCTION (this);
  m_plugins.clear ();
...
void
WmnWifiInterfaceMac::DoInitialize ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_beaconEnable)
    {
      ...
      m_beaconSendEvent = Simulator::Schedule (randomStart, &WmnWifiInterfaceMac::SendBeacon, this);
...
int64_t
WmnWifiInterfaceMac::AssignStreams (int64_t stream)
{
...
//-----------------------------------------------------------------------------
void
WmnWifiInterfaceMac::InstallPlugin (Ptr<WmnWifiInterfaceMacPlugin> plugin)
{
  NS_LOG_FUNCTION (this);
...
//-----------------------------------------------------------------------------
uint16_t
WmnWifiInterfaceMac::GetFrequencyChannel () const
{
  NS_LOG_FUNCTION (this);
...
void
WmnWifiInterfaceMac::SwitchFrequencyChannel (uint16_t new_id)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_phy != 0); // need PHY to set/get channel
  /**
...
//-----------------------------------------------------------------------------
void
WmnWifiInterfaceMac::ForwardDown (Ptr<const Packet> const_packet, Mac48Address from, Mac48Address to)
{
...
SupportedRates
WmnWifiInterfaceMac::GetSupportedRates () const
{
...
bool
WmnWifiInterfaceMac::CheckSupportedRates (SupportedRates rates) const
{
...
//-----------------------------------------------------------------------------
void
WmnWifiInterfaceMac::SetRandomStartDelay (Time interval)
{
  NS_LOG_FUNCTION (this << interval);
  m_randomStart = interval;
}
void
WmnWifiInterfaceMac::SetBeaconInterval (Time interval)
{
  NS_LOG_FUNCTION (this << interval);
  m_beaconInterval = interval;
}
Time
WmnWifiInterfaceMac::GetBeaconInterval () const
{
  return m_beaconInterval;
}
void
WmnWifiInterfaceMac::SetBeaconGeneration (bool enable)
{
  NS_LOG_FUNCTION (this << enable);
  m_beaconEnable = enable;
}
bool
WmnWifiInterfaceMac::GetBeaconGeneration () const
{
  return m_beaconSendEvent.IsRunning ();
}
Time
WmnWifiInterfaceMac::GetTbtt () const
{
  return m_tbtt;
}
void
WmnWifiInterfaceMac::ShiftTbtt (Time shift)
{
  ...
  m_beaconSendEvent = Simulator::Schedule (GetTbtt () - Simulator::Now (), &WmnWifiInterfaceMac::SendBeacon,
                                           this);
}
void
WmnWifiInterfaceMac::ScheduleNextBeacon ()
{
  m_tbtt += GetBeaconInterval ();
  m_beaconSendEvent = Simulator::Schedule (GetBeaconInterval (), &WmnWifiInterfaceMac::SendBeacon, this);
}
void
WmnWifiInterfaceMac::SendBeacon ()
{
  NS_LOG_FUNCTION (this);
  ...
  // Form & send beacon
  WmnWifiBeacon beacon (GetSsid (), GetSupportedRates (), m_beaconInterval.GetMicroSeconds ());
  ...
  m_dca->Queue (beacon.CreatePacket (), beacon.CreateHeader (GetAddress (), GetWmnPointAddress ()));

  ScheduleNextBeacon ();
}
void
WmnWifiInterfaceMac::Receive (Ptr<Packet> packet, WifiMacHeader const *hdr)
{
  // Process beacon
...
uint32_t
WmnWifiInterfaceMac::GetLinkMetric (Mac48Address peerAddress)
{
  uint32_t metric = 1;
...
void
WmnWifiInterfaceMac::SetLinkMetricCallback (Callback<uint32_t, Mac48Address, Ptr<WmnWifiInterfaceMac> > cb)
{
  m_linkMetricCallback = cb;
}
void
WmnWifiInterfaceMac::SetWmnPointAddress (Mac48Address a)
{
  m_mpAddress = a;
}
Mac48Address
WmnWifiInterfaceMac::GetWmnPointAddress () const
{
  return m_mpAddress;
}
//Statistics:
WmnWifiInterfaceMac::Statistics::Statistics () :
  recvBeacons (0), sentFrames (0), sentBytes (0), recvFrames (0), recvBytes (0)
{
}
void
WmnWifiInterfaceMac::Statistics::Print (std::ostream & os) const
{
  os << "<Statistics "
  /// \todo txBeacons
...
void
WmnWifiInterfaceMac::Report (std::ostream & os) const
{
  os << "<Interface "
...
void
WmnWifiInterfaceMac::ResetStats ()
{
  m_stats = Statistics ();
}

void
WmnWifiInterfaceMac::FinishConfigureStandard (enum WifiPhyStandard standard)
{
...
WifiPhyStandard
WmnWifiInterfaceMac::GetPhyStandard () const
{
  return m_standard;
}
} // namespace ns3
