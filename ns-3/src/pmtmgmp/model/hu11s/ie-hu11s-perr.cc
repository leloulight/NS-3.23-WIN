  ...
  for (unsigned int j = 0; j < numOfDest; j++)
    {
      ...
      MgmpProtocol::FailedDestination unit;
...
void
IePerr::AddAddressUnit (MgmpProtocol::FailedDestination unit)
{
...
{
  for (std::vector<MgmpProtocol::FailedDestination>::iterator i = m_addressUnits.begin (); i
