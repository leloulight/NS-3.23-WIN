...
Hu11sWmnCapability::Hu11sWmnCapability () :
...
uint8_t
Hu11sWmnCapability::GetSerializedSize () const
{
  return 1; 
}
uint8_t  
Hu11sWmnCapability::GetUint8 () const  //IEEE 802.11-2012 8.4.2.100.8 Wmn Capability 
{
  uint8_t result = 0;  
  if (acceptHuperLinks)
    {
      result |= 1 << 0; //The Accepting Additional Wmn Huperings subfield is set to 1 if the wmn STA is willing to establish additional wmn huperings   with other wmn STAs and set to 0 otherwise
    }
...
Buffer::Iterator
Hu11sWmnCapability::Serialize (Buffer::Iterator i) const
{
  i.WriteU8 (GetUint8 ());
  return i;
}
Buffer::Iterator
Hu11sWmnCapability::Deserialize (Buffer::Iterator i)
{
...
bool
Hu11sWmnCapability::Is (uint8_t cap, uint8_t n) const
{
  uint16_t mask = 1 << n;
  return (cap & mask);
}
...
Hu11sWmnCapability const&
IeConfiguration::WmnCapability ()
{
  return m_wmnCap;
}
bool
operator== (const Hu11sWmnCapability & a, const Hu11sWmnCapability & b)
{
