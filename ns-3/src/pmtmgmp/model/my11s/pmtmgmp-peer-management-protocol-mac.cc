...
#include "pmtmgmp-peer-management-protocol.h"
  ...
  if (header.IsAction ())
    {
     ...
     if ((actionValue.selfProtectedAction != WifiActionHeader::PEER_LINK_CONFIRM) && !fields.wmnId.IsEqual (
      ...
      //Peer Management element is the last element in this frame - so, we can use WmnInformationElementVector
      WmnInformationElementVector elements;
      packet->RemoveHeader (elements);
      peerElement = DynamicCast<IePeerManagement>(elements.FindFirst (IE11S_PMTMGMP_PEERING_MANAGEMENT));
