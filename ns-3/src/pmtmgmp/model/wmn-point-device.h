 ...
 * \ingroup wmn
 *
 * \brief Virtual net device modeling wmn point.
 *
 * Wmn point is a virtual net device which is responsible for
 *   - Aggregating and coordinating 1..* real devices -- wmn interfaces, see WmnInterfaceDevice class.
 *   - Hosting all wmn-related level 2 protocols.
 *
 * One of hosted L2 protocols must inplement L2RoutingProtocol interface and is used for packets forwarding.
 *
 * From the level 3 point of view WmnPointDevice is similar to BridgeNetDevice, but the packets,
 * which going through may be changed (because L2 protocols may require their own headers or tags).
 *
 * Attributes: \todo
 */
class WmnPointDevice : public NetDevice
{
public:
  /// Object type ID for NS3 object system
  static TypeId GetTypeId ();
  /// C-tor create empty (without interfaces and protocols) wmn point
  WmnPointDevice ();
  /// D-tor
  virtual ~WmnPointDevice ();
   ...
   *
   * \attention Only WmnPointDevice can have IP address, but not individual interfaces.
   */
  ...
  ///\name Protocols
  //\{
  /// Register routing protocol to be used. Protocol must be already installed on this wmn point.
  void SetRoutingProtocol (Ptr<WmnL2RoutingProtocol> protocol);
  /// Access current routing protocol
  Ptr<WmnL2RoutingProtocol> GetRoutingProtocol () const;
  //\}
...
private:
  ...
  /// Wmn point MAC address, supposed to be the address of the first added interface
  ...
  Ptr<WmnL2RoutingProtocol> m_routingProtocol;
