...
#ifndef WMN_WIFI_INTERFACE_MAC_H
#define WMN_WIFI_INTERFACE_MAC_H
 ...
 * \ingroup wmn
 *
 * \brief Basic MAC of wmn point Wi-Fi interface. Its function is extendable through plugins mechanism.
 *
 ...
 *
 */
class WmnWifiInterfaceMac : public RegularWifiMac
{
public:
  ...
  ///\name Each wmn point interfaces must know the wmn point address
  // \{
  void SetWmnPointAddress (Mac48Address);
  Mac48Address GetWmnPointAddress () const;
  // \}
  ///\name Beacons
  // \{
  ...
  void InstallPlugin (Ptr<WmnWifiInterfaceMacPlugin> plugin);
  // \}
  ...
  // \{
  void SetLinkMetricCallback (Callback<uint32_t, Mac48Address, Ptr<WmnWifiInterfaceMac> > cb);
...
private:
  typedef std::vector<Ptr<WmnWifiInterfaceMacPlugin> > PluginList;

  virtual void DoInitialize ();

  ///\name Wmn timing intervals
  // \{
  ...
  Callback<uint32_t, Mac48Address, Ptr<WmnWifiInterfaceMac> > m_linkMetricCallback;
  ///\name Statistics:
  // \{
  struct Statistics
  {
...
#endif /* WMN_WIFI_INTERFACE_MAC_H */
