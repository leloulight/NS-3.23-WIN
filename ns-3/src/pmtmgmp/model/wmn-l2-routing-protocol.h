...
#ifndef WMN_L2_ROUTING_PROTOCOL_H
#define WMN_L2_ROUTING_PROTOCOL_H
 ...
 * \ingroup wmn
 *
 * \brief Interface for L2 wmn routing protocol and wmn point communication.
 *
 * Every wmn routing protocol must implement this interface. Each wmn point (WmnPointDevice) is supposed
 * to know single L2RoutingProtocol to work with, see WmnPointDevice::SetRoutingProtocol ().
 *
 * This interface is similar to ipv4 routing protocol base class.
 */
class WmnL2RoutingProtocol : public Object
{
public:
  /// Never forget to support NS3 object model
  static TypeId GetTypeId ();
  /// virtual D-tor for subclasses
  virtual ~WmnL2RoutingProtocol ();
  /**
   ...
   *                    must be stored in proper tags (like in case of PMTMGMP, when WifiMacHeader
   ...
   *                    to WmnPointDevice looks like a pure packet with ethernet header
   *                    (i.e data + src +dst + protocol). The only special information addressed
   *                    to WmnPointDevice is an outcoming interface ID.
   *
   ...
   * WmnPointDevice must call this method when passing a packet to
   * upper layer.
  ...
  /// Set host wmn point, analog of SetNode (...) methods for upper layer protocols.
  void
  SetWmnPoint (Ptr<WmnPointDevice> mp);
  /// Each wmn protocol must be installed on the wmn point to work.
  Ptr<WmnPointDevice>
  GetWmnPoint () const;
protected:
  /// Host wmn point
  Ptr<WmnPointDevice> m_mp;
};
} // namespace ns3
#endif
